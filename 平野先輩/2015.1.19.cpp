...
#define sizenn_haba (6.1e6)//自然幅[Hz]
...
//いじる数字
#define MOT 1.0e7//MOT原子数
#define n1 1.45
#define r0 0.5e-3//MOT原子郡半径 単位[m]
#define r1 120e-6//開口半径 単位[m]
#define r2 3e-3//ファネル厚さ（水平方向） 単位[m]
#define HEIGHT (r2+2*r1+r0)//MOT中心高さz座標 単位[m]
#define TEMP 10.0E-6//MOT初期温度[K]
#define SAMPLE 1000
#define time_max 1.0//シミュレーション時間（原子の許容最大滞在時間）[s]
#define time_with 1e-1//time_max/box_number//フラックス強度評価時間[s]
#define gennsui_tyou (RAMUDA/(2*M_PI*sqrt(sin(45*M_PI/180)*sin(45*M_PI/180)*n1*n1-1))) //減衰長[m]
#define capplar_h 0e-3//カップラー高さ[m] カップラー無の時
//#define capplar_h r2//カップラー高さ[m]  カップラー有の時
//#define w2 r1//ビーム中空半径[m]
//#define w2 (r1+capplar_h)//ビーム中空半径[m] カップラー有の時
#define w1 2.25e-3//r2//ビーム半径[m]
//#define w1 (r2+capplar_h)//ビーム半径[m] カップラー有の時
#define beam_error_x 0.0e-6//中空ビームのx方向位置ずれ[m]
#define P (0.56*0.5)//ビームパワー[W]　円偏光の時*0.5
#define Enhance 4.6727107//入射光に対するエバネッセント光強度　＊＊円偏光の時
#define cappling_rate_fanel 1//ファネルのエネルギー伝達効率
#define cappling_rate_coupler 1//カップラーのエネルギー伝達効率
#define cappling_rate cappling_rate_fanel*cappling_rate_coupler//システム全体のエネルギー伝達効率
#define delta (1.2e9)//レーザー周波数離調[Hz]
#define dt 1.0e-5
...
int reflection_jadge(double xa, double ya, double za, double vxa, double vya, double vza,int ff);
double U_laser(double xa, double ya,int ff);
...
double I_laser(double xa, double ya,int ff);
double I_laser_fanel(double xa, double ya,int ff);
...
int main(){
	...
	int beam_No = 1;//0:アキシコン　1:ガウシアンフィルター
	int jyourann_No = 1;//0擾乱無し　1:擾乱有
	...
	int pp;
	for (pp = 0; pp < simulate_No+1;pp++){

		std::string para_name = "w2";
		w2 = with*pp;
		double para = w2;
	...
	*/
	...
	for (i = 0; i < SAMPLE; i++){
		...
		for (;;){
			...
			if (sqrt(x*x + y*y) > r1){
				...
				x += vx*dt_1;
				y += vy*dt_1;
				z += vz*dt_1 - G*dt_1*dt_1 / 2.0;
				vz += -G*dt_1;
				t += dt_1;

			}

			else if (jyourann_No == 1){
				//漏れ励起光による散逸力

				double F, arufa;
				F = h_bar*(2 * M_PI) / RAMUDA*sizenn_haba / 2 * (I_laser(x, y, beam_No) / (I_laser(x, y, beam_No) + Is*(1 + (4 * delta *delta) / (sizenn_haba*sizenn_haba))));
				arufa = F / MASS;
			...
			case 3:
				...
				case 1:
					...
					//反射をするかどうか判定
					loss_jadge = reflection_jadge(x, y, z, vx, vy, vz, beam_No);
					if (loss_jadge == 1){
	...
	printf("loss_count\t%f\t\tgain_count\t%f\t\ttime_out_count\t%f\n", loss_count, gain_count, time_out_count);
	...
	ofs1 << "para = ," << para << ", flux_1s = ," << flux_1s << ", flux_intensity_max = ," << flux_intensity_max << ",";	// 変わる
...
int reflection_jadge(double x, double y, double z, double vx, double vy, double vz,int ff){
	double U_Rb;
	int loss_jadge_1;
	U_Rb = MASS / (2 * (x*x + y*y + z*z))*pow(-x*vx - y*vy + z*vz, 2);//*J_to_K;
	//printf("%f\t%f\n", U_Rb*J_to_K, U_laser(x, y)*J_to_K);
	if (U_Rb>U_laser(x, y,ff)){
		loss_jadge_1 = 1;
	}
	else loss_jadge_1 = 0;
...
double I_laser(double xa, double ya,int ff){
	...
	if (ff==1){
		Ixy_laser = (2 * P) / (M_PI*(w1*w1))*(exp(-(2 * ((xa - beam_error_x)*(xa - beam_error_x) + ya*ya)) / (w1*w1)) - exp(-(2 * ((xa - beam_error_x)*(xa - beam_error_x) + ya*ya)) / (w2*w2)));
	}
	else {
		Ixy_laser = (2 * P) / (M_PI*(w1*w1))*(exp(-(2 * ((xa - beam_error_x)*(xa - beam_error_x) + ya*ya)) / (w1*w1)) );
	}
...
double I_laser_fanel(double xa, double ya,int ff){
	...
	if (sqrt(xa*xa + ya*ya) >= (r1 + capplar_h) && sqrt(xa*xa + ya*ya) <= (r2 + r1)){
		Ixy_fanel = I_laser(xa, ya,ff);
	}
	else if (sqrt(xa*xa + ya*ya) >= (r1) && sqrt(xa*xa + ya*ya)<(r1 + capplar_h)){
		Ixy_fanel = I_laser(xa*(1 + r2 / sqrt(xa*xa + ya*ya)), ya*(1 + r2 / (sqrt(xa*xa + ya*ya))),ff);
	}
	else Ixy_fanel = 0;
	//printf("%f\n", Ixy_fanel);
	return Ixy_fanel;
