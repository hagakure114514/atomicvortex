...
//===ATTENTION!============================================================================================================================================================
	//If you want to know the detail of this program, please refer to Kimura's bachelor thesis and Kimura's folder in the laboratory PC.

	//If you have any questions, please send an e-mail to sb103_pic_sh_115og*yahoo.co.jp (Kimura's e-mail address, change "*" into "@").

	//### When you change the values of mass(Rb) or temperature, change the number used in yj in max_boltz_velo_x(y,z).

	//### When you change the MOT radius, change the number used in yj in gauss_pos_x(y,z) and also update Gaussian random no.

	//### When you change the beam waist, change the condition of loss judging.

	//### If you change the initial eigenstate, change the initial value of sta1 or sta2.


	//When you compile this file, check whether the path of 'gnuplot.exe', 'rand_gauss.csv' and 'rand_maxbol.csv' is correct.

	//Be careful! If you write "1/2", this is culculated as a division of integers,so the answer becomes 0.
	//(In case of a calculation of integers, the numbers after the decimal point of the answer are ignored.)
	//But if you write "1.0/2.0", this is calculated as a division of double type numbers, so the answer becomes 0.500000.

	//gnuplot.exe sometimes stops for some reason, but I could not find the concrete reason.
//===ATTENTION! <end>======================================================================================================================================================
...
//===Constants that can be changed=========================================================================================================================================
	...
	#define r_simu 2.0*waist								//maximum radious up to which an atom is traced
	#define r_judge waist/sqrt(2.0)							//radious to judge whther an atom is TRAPPED or LOSS (the radious of the top of TEM)
	#define delta 1.0e+9*2.0*M_PI							//detuning[rad/s]
	#define power 300.0e-3									//beam power[W]
	...
	//====fixed====
		...
		#define dt 5.0e-5									//time interval[s]
	...
	//############# temporary sentence for power loop ########################
		#define min_branch 0.01								//minimum value of when varying branching ratio[-]
		#define max_branch 0.99								//maximum value of when varying branching ratio[-]
		#define d_branch 0.01								//increment of branching ratio
		#define digit_branch 2								//how many digits "branch" has after the decimal point
	//############# temporary sentence for power loop <end> ##################
	
	//====depending on variables above====
		#define I_0 power/(M_PI*waist*waist/2.0)			//[W/m^2]
		#define lambda c/(384.23e+12+delta/(2.0*M_PI))		//wavelength of detuned D2 line[m]
		#define lambda1 c/(384.23e+12)						//wavelength between |e> and |g1>[m]
		#define lambda2 c/(384.23e+12)						//wavelength between |e> and |g2>[m]
...
//===Variables for statements, temporary variables=========================================================================================================================
	...
	int sta1 = 0;					//how many times the atom has been in |1>
	int sta2 = 0;					//how many times the atom has been in |2>
	...
	char var_f[20];					//format specifier for file names
	//############# temporary sentence for power loop ########################
		int vara = 1;					//values of variables for file names
	//############# temporary sentence for power loop <end> ##################
	...
	int count = 1;					//how many conditions have done
	...
	//====file pointer====
		...
		FILE *x_y_z_velo;				//velocities in x direction
		FILE *r_max;					//maximum radii
		FILE *r_phi_velo;				//velocities in radial direction
		FILE *momentum;					//momentums
		FILE *angmomentum;				//angular momentums
		FILE *dipole;					//dipole forces
		FILE *passed_time;				//passed times till the atom reaches the substrate
		...
		FILE *f_position;				//final position of j-loop
...
//===Function Definition (prototype)=======================================================================================================================================
	...
	double U1_opt( double z, double d, double g1, double Is1 );
	double U2_opt( double z, double d, double g2, double Is2 );
	double dipole_f1( double z, double w, double d, double g1, double Is1 );
	double dipole_f2( double z, double w, double d, double g2, double Is2 );
	double dipole_f_x1( double x, double y, double z, double w, double g1, double Is1 );
	double dipole_f_y1( double x, double y, double z, double w, double g1, double Is1 );
	double dipole_f_x2( double x, double y, double z, double w, double g2, double Is2 );
	double dipole_f_y2( double x, double y, double z, double w, double g2, double Is2 );
...
int main(){
	...
	//===def of variables==================================================================================================================================================
		...
		double branch = 0.0;									//branching ratio into |1>(lower hyperfine ground state) (based on "Gravitational laser trap for atoms with evanescent-wave cooling" pp.654, 661)
	...
	//===for loop (repeat changing one variable)===========================================================================================================================
		for( branch = min_branch; branch <=max_branch+0.005 ; branch += d_branch, vara += 1, count++ ){

			//############# temporary sentence for power loop ##################
				double gamma1 = 38.1e+6*branch;									//spontaneous decay rate between |e> and |g1>[rad/s]
				double gamma2 = 38.1e+6*(1.0-branch);							//spontaneous decay rate between |e> and |g2>[rad/s]
				double I_s1 = M_PI*h*c*gamma1/(3.0*lambda1*lambda1*lambda1);	//saturation intensity(|1>)[W/m^2]
				double I_s2 = M_PI*h*c*gamma2/(3.0*lambda2*lambda2*lambda2);	//saturation intensity(|2>)[W/m^2]
				sprintf( var_f, "%%.%df", digit_branch );						//store "%.2f" into var_f (ex.digit_branch = 2)
				sprintf( var, var_f, branch );
			//############# temporary sentence for power loop <end>#############
			...
			//===for loop (repeat the number of SAMPLES)===================================================================================================================
				...
				//===initialize variables and arrays with 0================================================================================================================
					...
					theta2 = 0.0, phi2 = 0.0;						//spherical coord. only to make spherical random no. to determine the direction of the photon emitted when spontaneous emission occurs
				...
				//===initialize variables and arrays=======================================================================================================================
					...
					//dipole force
					if( state==1 ){
						dipole_f = dipole_f1(int_TEM(x,y,I_0,waist),d_int_TEM(x,y,I_0,waist),delta,gamma1,I_s1);
					}else if( state==2 ){
						dipole_f = dipole_f2(int_TEM(x,y,I_0,waist),d_int_TEM(x,y,I_0,waist),delta,gamma2,I_s2);
					}

					//optical potential
					if( state==1 ){
						U_dip = U1_opt(int_TEM(x,y,I_0,waist), delta, gamma1, I_s1);
					}else if( state==2 ){
						U_dip = U2_opt(int_TEM(x,y,I_0,waist), delta, gamma2, I_s2);
					}

					//kinetic + potential
					if( state==1 ){
						Ene = 1.0/2.0*mass*v*v + U1_opt(int_TEM(x,y,I_0,waist), delta, gamma1, I_s1) + mass*G*z;
					}else if( state==2 ){
						Ene = 1.0/2.0*mass*v*v + U2_opt(int_TEM(x,y,I_0,waist), delta, gamma2, I_s2) + mass*G*z;
					}
					...
					//====insert variables into arrays for plotting============================================
					if(gnu==1){
				...
				//===open files for outputting values <end>================================================================================================================
					...
					//===for loop (repeat the number of exporting times)===================================================================================================
						for( int j=1; j<=jloop; j++ ){		
							...
							//===output initial values and variations into csv files=======================================================================================
								if( j==1 ){		//outuput initial values into csv files
									fprintf( i_x_y_z, "%.16e,%.16e,%.16e\n", x, y, z );	//output initial radii (on x-y plane) into i_radius.csv
									fprintf( i_velo_on_xy, "%.16e\n", sqrt(vx*vx+vy*vy) );				//output initial velocities (on x-y plane) into i_velocity_on_xy.csv
									fprintf( i_velo_x_y_z, "%.16e,%.16e,%.16e\n", vx, vy, vz );			//output initial velocities in x, y and z direction into i_velocity_x_y_z.csv
									fprintf( i_velo, "%.16e\n", v );									//output initial velocities into i_velocity.csv
									fprintf( i_velo_r_phi, "%.16e,%.16e\n", vr, vphi );					//output initial radial velocities into i_velocity_r.csv
									fprintf( i_angmomentum, "%.16e\n", ang_mom );						//output initial angular momentum into i_ang_mom.csv
									fprintf( i_kinetic, "%.16e\n", kinetic_E );							//output initial kinetic energy into i_kinetic.csv
									if( csv_out==1 ){
									fprintf( dEnergy, "%.16e\n", dEne );								//output difference of kinetic + potential energy into i_dEnergy.csv
									fprintf( dMomentum, "%.16e\n", dMom );									//output difference of momentum into .csv
									fprintf( dMom_x_y_z, "%.16e,%.16e,%.16e\n", dMom_x, dMom_y, dMom_z );//output difference of momentum in x, y and z direction into i_dMomentum.csv
									}
								}
							...
							//===output values into csv files==============================================================================================================
								if( csv_out==1 ){
									fprintf( x_y_z_coord, "%.16e,%.16e,%.16e\n", x, y, z );//outuput x coord. into x_%d.csv (%d = i)
									fprintf( velo, "%.16e\n", v );						//outuput velocity into velocity_%d.csv (%d = i)
									fprintf( x_y_z_velo, "%.16e,%.16e,%.16e\n", vx, vy, vz );//outuput velocity into x_velocity_%d.csv (%d = i)
									fprintf( r_phi_coord, "%.16e,%.16e\n", rad, phi );	//outuput radius into r_%d.csv (%d = i)
									fprintf( r_phi_velo, "%.16e,%.16e\n", vr, vphi );	//outuput radial velocity into r_velocity_%d.csv (%d = i)
									fprintf( kinetic, "%.16e\n", kinetic_E );			//outuput kinetic energy into phi_velocity_%d.csv (%d = i)
									fprintf( momentum, "%.16e\n", mom );				//outuput momentum into _%d.csv (%d = i)
									fprintf( Mom_x_y_z, "%.16e,%.16e,%.16e\n", Mom_x, Mom_y, Mom_z );//outuput momentum in x, y and z direction into _%d.csv (%d = i)
									fprintf( angmomentum, "%.16e\n", ang_mom );			//outuput angular momentum into phi_velocity_%d.csv (%d = i)
									fprintf( dipole, "%.16e\n", dipole_f );				//outuput dipole force into phi_velocity_%d.csv (%d = i)
									fprintf( Udip, "%.16e\n", U_dip );					//outuput optical potential into opt_pot_%d.csv (%d = i)
									fprintf( Ugrav, "%.16e\n", mass*G*z );				//outuput optical potential into opt_pot_%d.csv (%d = i)
									fprintf( Energy, "%.16e\n", Ene );					//outuput kinetic + potential energy into Energy_%d.csv (%d = i)
									fprintf( eigenstate, "%d\n", state );				//outuput energy eigenstate into eigstate_%d.csv (%d = i)
									fprintf( psp, "%.16e,%.16e\n", psp1, psp2 );		//outuput transition probability into _%d.csv (%d = i)
								}
										...
										if( sp==1 ){
											if( r1[sta1+1]<=psp1 && I_0!=0.0 ){
												//===advance velosity for dt based on absorption of laser photonand on spontaneous emission================================
													...
													vx_tt = vx; vy_tt = vy; vz_tt = vz; v_tt = sqrt(vx*vx+vy*vy+vz*vz);//substitute velocities after spontaneous emission into temporary variables
													spon++;
												...
												//===branch into a eigenstate based on branching ratio=====================================================================
													if( r5[spon+1]<=branch ){
														...
														//===advance position, velosity, time etc. for dt on |1>===========================================================
															a_dx = dipole_f_x1(x,y,int_TEM(x,y,I_0,waist),d_int_TEM(x,y,I_0,waist),gamma1,I_s1)/mass;	//acceleration by dipole force (x component)
															a_dy = dipole_f_y1(x,y,int_TEM(x,y,I_0,waist),d_int_TEM(x,y,I_0,waist),gamma1,I_s1)/mass;	//acceleration by dipole force (y component)
															...
															U_dip = U1_opt(int_TEM(x,y,I_0,waist), delta, gamma1, I_s1);
															Ene = 1.0/2.0*mass*v*v + U1_opt(int_TEM(x,y,I_0,waist), delta, gamma1, I_s1) + mass*G*z;
															s1 = int_TEM(x_t,y_t,I_0,waist)/(I_s1*(1.0+4.0*delta*delta/(gamma*gamma)));
															s2 = int_TEM(x_t,y_t,I_0,waist)/(I_s2*(1.0+4.0*(delta+delta_hfs)*(delta+delta_hfs)/(gamma*gamma)));
															psp1 = gamma1*s1/2.0*dt;
															psp2 = gamma2*s2/2.0*dt;
															t += dt;
															sta1++;
														...
														//===advance position, velosity, time etc. for dt on |2>===========================================================
															a_dx = dipole_f_x2(x,y,int_TEM(x,y,I_0,waist),d_int_TEM(x,y,I_0,waist),gamma2,I_s2)/mass;	//acceleration by dipole force (x component)
															a_dy = dipole_f_y2(x,y,int_TEM(x,y,I_0,waist),d_int_TEM(x,y,I_0,waist),gamma2,I_s2)/mass;	//acceleration by dipole force (y component)
															x += vx*dt + (1.0/2.0)*a_dx*dt*dt; y += vy*dt + (1.0/2.0)*a_dy*dt*dt; z += vz*dt - (1.0/2.0)*G*dt*dt;
															vx += a_dx*dt; vy += a_dy*dt; vz += - G*dt;
															v = sqrt( vx*vx + vy*vy + vz*vz );
															rad = sqrt( x*x + y*y );
								...
								if( sqrt(x*x+y*y) > r_simu || z < -0.26 || j==jloop ) {		//If the atom is out of beam or on the substrate or if j==jloop, this program quits to simulate the current atom.
									...
									if( csv_out==1 ){
										if( state_t==1&&state==1 ){
											...
											fprintf( dEnergy, "%.16e\n", 1.0/2.0*mass*(v*v-v_t*v_t) + (U1_opt(int_TEM(x,y,I_0,waist), delta, gamma1, I_s1)-U1_opt(int_TEM(x_t,y_t,I_0,waist), delta, gamma1, I_s1)) + mass*G*(z-z_t) );
											fprintf( dMom_x_y_z, "%.16e,%.16e,%.16e\n", mass*vx - (mass*vx_t + dipole_f_x1( x_t, y_t, int_TEM(x_t,y_t,I_0,waist), d_int_TEM(x_t,y_t,I_0,waist), gamma1, I_s1 )*dt), mass*vy - (mass*vy_t + dipole_f_y1( x_t, y_t, int_TEM(x_t,y_t,I_0,waist), d_int_TEM(x_t,y_t,I_0,waist), gamma1, I_s1 )*dt), mass*vz - (mass*vz_t - mass*G*dt) );
										}else if( state_t==2&&state==2 ){
											//when a spontaneous emission has not occured
											fprintf( dEnergy, "%.16e\n", 1.0/2.0*mass*(v*v-v_t*v_t) + (U2_opt(int_TEM(x,y,I_0,waist), delta, gamma2, I_s2)-U2_opt(int_TEM(x_t,y_t,I_0,waist), delta, gamma2, I_s2)) + mass*G*(z-z_t) );
											fprintf( dMom_x_y_z, "%.16e,%.16e,%.16e\n", mass*vx - (mass*vx_t + dipole_f_x2( x_t, y_t, int_TEM(x_t,y_t,I_0,waist), d_int_TEM(x_t,y_t,I_0,waist), gamma2, I_s2 )*dt), mass*vy - (mass*vy_t + dipole_f_y2( x_t, y_t, int_TEM(x_t,y_t,I_0,waist), d_int_TEM(x_t,y_t,I_0,waist), gamma2, I_s2 )*dt), mass*vz - (mass*vz_t - mass*G*dt) );
										}else if( state_t==1&&state==2 ){
											//when a spontaneous emission has occured
											fprintf( dEnergy, "%.16e\n", hbar*2.0*M_PI*c/lambda + 1.0/2.0*mass*(v_tt*v_tt-v_t*v_t) + (U2_opt(int_TEM(x_t,y_t,I_0,waist), delta, gamma2, I_s2)-U1_opt(int_TEM(x_t,y_t,I_0,waist), delta, gamma1, I_s1)) + (U1_opt(int_TEM(x_t,y_t,I_0,waist), delta, gamma1, I_s1)-U2_opt(int_TEM(x_t,y_t,I_0,waist), delta, gamma2, I_s2)) + 1.0/2.0*mass*(v*v-v_tt*v_tt) + (U2_opt(int_TEM(x,y,I_0,waist), delta, gamma2, I_s2)-U2_opt(int_TEM(x_t,y_t,I_0,waist), delta, gamma2, I_s2)) + mass*G*(z-z_t) );
											fprintf( dMom_x_y_z, "%.16e,%.16e,%.16e\n", mass*vx - (mass*vx_t + hbar*k_wave*sin(theta2)*cos(phi2) + dipole_f_x2( x_t, y_t, int_TEM(x_t,y_t,I_0,waist), d_int_TEM(x_t,y_t,I_0,waist), gamma2, I_s2 )*dt), mass*vy - (mass*vy_t + hbar*k_wave*sin(theta2)*sin(phi2) +dipole_f_y2( x_t, y_t, int_TEM(x_t,y_t,I_0,waist), d_int_TEM(x_t,y_t,I_0,waist), gamma2, I_s2 )*dt), mass*vz - (mass*vz_t + hbar*k_wave*cos(theta2) - mass*G*dt) );
										}else if( state_t==2&&state==1 ){
											//when a spontaneous emission has occured
											fprintf( dEnergy, "%.16e\n", hbar*2.0*M_PI*c/lambda + 1.0/2.0*mass*(v_tt*v_tt-v_t*v_t) + (U1_opt(int_TEM(x_t,y_t,I_0,waist), delta, gamma1, I_s1)-U2_opt(int_TEM(x_t,y_t,I_0,waist), delta, gamma2, I_s2)) + (U2_opt(int_TEM(x_t,y_t,I_0,waist), delta, gamma2, I_s2)-U1_opt(int_TEM(x_t,y_t,I_0,waist), delta, gamma1, I_s1)) + 1.0/2.0*mass*(v*v-v_tt*v_tt) + (U1_opt(int_TEM(x,y,I_0,waist), delta, gamma1, I_s1)-U1_opt(int_TEM(x_t,y_t,I_0,waist), delta, gamma1, I_s1)) + mass*G*(z-z_t) );
											fprintf( dMom_x_y_z, "%.16e,%.16e,%.16e\n", mass*vx - (mass*vx_t + hbar*k_wave*sin(theta2)*cos(phi2) + dipole_f_x1( x_t, y_t, int_TEM(x_t,y_t,I_0,waist), d_int_TEM(x_t,y_t,I_0,waist), gamma1, I_s1 )*dt), mass*vy - (mass*vy_t + hbar*k_wave*sin(theta2)*sin(phi2) +dipole_f_y1( x_t, y_t, int_TEM(x_t,y_t,I_0,waist), d_int_TEM(x_t,y_t,I_0,waist), gamma1, I_s1 )*dt), mass*vz - (mass*vz_t + hbar*k_wave*cos(theta2) - mass*G*dt) );
										}
										fprintf( dMomentum, "%.16e\n", mass*v - mass*v_t );
									}
					...
					//===close csv files if SAMPLE is at the end===========================================================================================================
						if( i==SAMPLE ){		//If SAMPLE loop (i loop) is at the end, csv files are closed.
							//====write data by compulsion into csv files if there are unwrittend data on the RAM====
...
//===Function Definiton====================================================================================================================================================
	...
	double U1_opt( double z, double d, double g1, double Is1 ){	//Intensity, detuning and gamma1 must be substituted into z, d and g1, respectively.
		return hbar*d/2.0*log( 1.0 + 1.0/Is1*g1*g1/(g1*g1 + 4.0*d*d)*z );
	}

	//optical potential on |2>
	double U2_opt( double z, double d, double g2, double Is2 ){	//Intensity, detuning and gamma2 must be substituted into z, d and g2, respectively.
		return hbar*(d+delta_hfs)/2.0*log( 1.0 + 1.0/Is2*g2*g2/(g2*g2 + 4.0*(d+delta_hfs)*(d+delta_hfs))*z );
	}

	//dipole force on |1>
	double dipole_f1( double z, double w, double d, double g1, double Is1 ){		//Intensity, differentiated intensity, detuning and gamma1 must be substituted into z, w, d and g1, respectively.
		return -hbar*d*1.0/(2.0*Is1)*g1*g1*w/( g1*g1 + 4.0*d*d + 1.0/Is1*g1*g1*z );
	}

	//dipole force on |2>
	double dipole_f2( double z, double w, double d, double g2, double Is2 ){		//Intensity, differentiated intensity, detuning and gamma2 must be substituted into z, w, d and g2, respectively.
		return -hbar*( d + delta_hfs )*1.0/(2.0*Is2)*g2*g2*w/( g2*g2 + 4.0*(d+delta_hfs)*(d+delta_hfs) + 1.0/Is2*g2*g2*z );
	}

	//x component of dipole force on |1>
	double dipole_f_x1( double x, double y, double z, double w, double g1, double Is1 ){
		return dipole_f1( z,w,delta,g1,Is1 )*x/sqrt(x*x+y*y);
	}

	//y component of dipole force on |1>
	double dipole_f_y1( double x, double y, double z, double w, double g1, double Is1 ){
		return dipole_f1( z,w,delta,g1,Is1 )*y/sqrt(x*x+y*y);
	}

	//x component of dipole force on |2>
	double dipole_f_x2( double x, double y, double z, double w, double g2, double Is2 ){
		return dipole_f2( z,w,delta,g2,Is2 )*x/sqrt(x*x+y*y);
	}

	//y component of dipole force on |2>
	double dipole_f_y2( double x, double y, double z, double w, double g2, double Is2 ){
		return dipole_f2( z,w,delta,g2,Is2 )*y/sqrt(x*x+y*y);
	}
