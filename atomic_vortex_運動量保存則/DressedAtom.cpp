...
void DressedAtom::recoil_diss_pm(atom* obj)
{
	...
	if(flag_sp == 2) {
		...
		obj->p_rot += hbar * (-(double)l_pm/obj->radius);
...
{
	obj->r.x += ( 1.0/2.0 * (obj->v.vx + obj->v_pre.vx) - obj->p_rot * sin(obj->phi) / mass ) * dt + 1.0/2.0 * obj->acc_x * dt * dt;
	obj->r.y += ( 1.0/2.0 * (obj->v.vy + obj->v_pre.vy) + obj->p_rot * cos(obj->phi) / mass ) * dt + 1.0/2.0 * obj->acc_y * dt * dt;
...
{
	double vphi = -(obj->v_pre.vx) * sin(obj->phi) + (obj->v_pre.vy) * cos(obj->phi) + obj->p_rot / mass;
...
void DressedAtom::recoil_diss(atom* obj)
{
	...
	if(flag_sp == 1){
		...
		dip_sin2(&sp_theata);
		obj->p_rot += hbar * (double)l / obj->radius;
